...
<head>
	...
	$(document).ready(function(){
		$('tr').click(function(){
			...
			if(!isNaN(bNum)){
				location.href = '${conPath}/nContent_view.do?nNum='+nNum+'&pageNum=${pageNum}';
			}
		});
	});
	</script>
...
<body>
	...
	<div class="writeBtn">
		<table>
			<tr><td>
						<c:if test="${not empty admin }"><a href="${conPath }/nWrite_view.do">글쓰기</a></c:if>
			</td></tr>
		</table>
	</div>
	<div class="boardlist">
		...
		<tbody>
			...
			<c:if test="${totCnt!=0 }">
				<c:forEach items="${nlist }" var="ndto">
					<tr><td>${ndto.nNum }</td>
							<td class="left">
								<a href="${conPath }/nContent_view.do?nNum=${ndto.nNum}&pageNum=${pageNum}">${ndto.nTitle }</a>
							</td>
							<td>${ndto.aId }</td>
							<td>${ndto.nHIT }</td>
	...
	<div class="paging">
		<c:if test="${startPage > BLOCKSIZE }">
			[ <a href="${conPath }/nList.do?pageNum=${startPage-1}"> 이전 </a> ]
		</c:if>
		...
		<c:if test="${endPage<pageCnt }">
		  [ <a href="${conPath }/nList.do?pageNum=${endPage+1}"> 다음 </a> ]
		</c:if>
	</div>
</div>
</div>
